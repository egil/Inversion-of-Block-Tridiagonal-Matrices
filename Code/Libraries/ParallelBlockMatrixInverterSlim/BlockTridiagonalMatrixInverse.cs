...
{
    public class BlockTridiagonalMatrixInverse<T> : IProducer<IProducer<Action>>
    {
        #region Fields

        private static TraceSource Log = new TraceSource("BlockTridiagonalMatrixInverse");
        ...
        public BlockTridiagonalMatrixInverse(BlockTridiagonalMatrix<T> data, int tileSize, out BlockTridiagonalMatrix<T> result)
        {
            _input = data;
            _tileSize = tileSize;
            ...
            _g = Helpers.Init<OperationResult<T>>(N + 1, 3);
            _glu = new OperationResult<T>[N + 1];
            _gSum = new OperationResult<T>[N + 1];

            _result = result = new BlockTridiagonalMatrix<T>(data.Size);

            _gen = new OperationEnumerator<AbstractOperation<OpType>>(FormulaeGenerator(N), 1 /* max queue length */);
        }
        ...
        private IProducer<Action> GenerateProducer(AbstractOperation<OpType> op)
        {
            ...
            //Debug.WriteLine(Thread.CurrentThread.Name + " [ BlockTridiagonalMatrixInverse: " + op + "]");
            //Log.TraceInformation(Thread.CurrentThread.Name + " [ BlockTridiagonalMatrixInverse: " + op + "]");
            ...
            switch (op.OP)
            {
                ...
                case OpType.cL:
                    ...
                    return new MinusMatrixInverseMatrixMultiply<T>(_a[op.I + 1][0], _dllu[op.I], out _cl[op.I]);

                case OpType.cR:
                    // calculate cr_i = -a_i-1,i * (dr_i,i)^-1
                    return new MinusMatrixInverseMatrixMultiply<T>(_a[op.I - 1][2], _drlu[op.I], out _cr[op.I]);
        ...
        {            
            switch (op.OP)
            {
                ...
                case OpType.cL:
                    return _dllu[op.I] != null && 
                           _a[op.I + 1][0] != null;

                case OpType.cR:

                    return _drlu[op.I] != null &&
                           _a[op.I - 1][2] != null;

                case OpType.dL:
                    if (op.I == 1)
                        return _a[op.I][1] != null;
                    return _a[op.I][1] != null &&
                           _cl[op.I - 1] != null &&
                           _a[op.I - 1][2] != null;

                case OpType.dLLU:
                    return _dl[op.I] != null;
                ...
                case OpType.g:
                    if (op.I == op.J)
                        return _glu[op.I] != null;
                    else
                    {
                        var ret = _g[op.I][1] != null;
                        if (op.I < op.J)
                        {
                ...
                case OpType.Untile:
                    ...
                    {
                        if (i > 1)
                            retval = retval && _g[i][0] != null;

                        retval = retval && _g[i][1] != null;

                        if (i < length)
                            retval = retval && _g[i][2] != null;
                    }
                    return retval;
